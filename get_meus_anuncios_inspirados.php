...
$sql = " SELECT id_anuncio, titulo, descricao, cidade, estado, categoria, valor, foto, DATE_FORMAT(anuncios.data_publicacao, '%d/%m/%Y - %H:%i') AS data_inclusao_formatada FROM anuncios WHERE id_usuario = $id_usuario and data_evento < '".date("Y-m-d")."' and status = '' ORDER BY data_publicacao DESC ";
  ...
  if($resultado_id){
    ...
    echo'<div class="caixa-anuncio-inspirados group17">
        <div class="row">
        <div class="col-sm-12">
        <div class="titulo">
            <B>'.$registro['titulo'].'</B>
